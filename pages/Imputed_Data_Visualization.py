...
def time_series_data_eda(dataset: pd.DataFrame, id_column: str = 'Chiffre', date_column = 'Date', patient_id: str = None):
...
st.header("Imputed Data Visualization")
col1, col2 = st.columns([1,1])

with col1:
    long_format_dataset = pd.read_csv(f"data/dynops_imputed_dataset_{date.today().strftime('%d-%m-%y')}.csv")
...
if selected_variable == 'Chiffre':
    ...
    if not time_series_data.empty:
        ...
        fig.update_yaxes(range=[0, 3.5], dtick=1)
        st.plotly_chart(fig)
    else:
    ...
    if not time_series_data.empty:
        ...
        y_max = 3 * len(phqvars)
        fig.update_yaxes(range=[0, y_max+0.5], dtick=1)
        st.plotly_chart(fig)
    else:
        st.warning("No data available for this patient.")
